...
COLLECTION_PERIOD_HOURS = 24            # Сколько часов просто собирать стакан перед стартом торгов (напр. 24 или 48)
MIN_EVENTS_TO_START_TRADING = 30        # Минимальное кол-во зафиксированных импульсов для перехода к торговле
...
INITIAL_WALL_THRESHOLD_USD = 1_000_000  # Детектим потенциальные стены от $1M во время обучения
...
def get_adaptive_settings(state):
    ...
    # Если прошло мало времени и собрано мало данных — режим накопления
    if not (is_time_passed or is_enough_data):
        return INITIAL_WALL_THRESHOLD_USD, False, elapsed_hours

    # Берем стены, пробой которых привел к импульсу >= TAKE_PROFIT_USD
    successful_walls = [e["peak_volume_usd"] for e in history if e["is_successful_breakout"]]
    
    if len(successful_walls) < 5:
        # Если пробоев мало, берем средний объем от всех зарегистрированных стен
        ...
        # Порог как 85% от среднего размера успешных стен
...
async def start_orderbook_ws():
    ...
    send_telegram(
        f"⚡ <b>Wall Breakout Bot v3.1 Запущен!</b>\n"
            ...
            async with websockets.connect(ws_url, ping_interval=20, ping_timeout=20) as ws:
                ...
                async for message in ws:
                    ...
                    for breakout in active_breakouts[:]:
                        ...
                        # Через 15 сек логируем силу импульса
                    ...
                    # -------------------------------------------------
                    # А) Детект пробоев ASKS
                    for ask_lvl, ask_vol in g_asks.items():
                        if ask_lvl < current_price: continue
                        if ask_vol >= wall_threshold:
                            tracked_ask_walls[ask_lvl] = max(tracked_ask_walls.get(ask_lvl, 0), ask_vol)
                        elif ask_lvl in tracked_ask_walls and ask_vol <= EATEN_WALL_THRESHOLD_USD:
                            ...
                            if time.time() > level_cooldowns.get(ask_lvl, 0):
                                ...
                                # Вход исполняется ТОЛЬКО если включен режим торговли
                    ...
                    # Б) Детект пробоев BIDS
                    for bid_lvl, bid_vol in g_bids.items():
                        if bid_lvl > current_price: continue
                        if bid_vol >= wall_threshold:
                            tracked_bid_walls[bid_lvl] = max(tracked_bid_walls.get(bid_lvl, 0), bid_vol)
                        elif bid_lvl in tracked_bid_walls and bid_vol <= EATEN_WALL_THRESHOLD_USD:
